...
const defaultNavItems: iNavItem[] = [
	{
		...
		imgSrc: "https://images.unsplash.com/photo-1497215728101-856f4ea42174?q=80&w=2070&auto=format&fit=crop",
	},
	{
		heading: "Components",
		href: "/components",
	...
	},
	{
		...
		imgSrc: "https://images.unsplash.com/photo-1423666639041-f56000c27a9a?q=80&w=2074&auto=format&fit=crop",
	},
];
...
const NavLink: React.FC<iNavLinkProps> = ({
	...
	return (
		...
		>
			<a ref={ref} onMouseMove={handleMouseMove} href={href} className="w-full flex" {...linkProps}>
	...
	return (
		<>
			<div className="relative">
				<div
					onClick={handleClick}
					className="fixed right-0 top-0 m-5 z-50 w-16 h-16 rounded-full flex items-center justify-center cursor-pointer bg-white shadow-xl hover:scale-105 transition-transform"
				>
					<div className="relative w-8 h-6 flex flex-col justify-between items-center">
						<span
							className={`block h-[2px] w-7 bg-black transition-transform duration-300 ${isActive ? "rotate-45 translate-y-3" : ""}`}
						></span>
						<span
							className={`block h-[2px] w-7 bg-black transition-opacity duration-300 ${isActive ? "opacity-0" : ""}`}
						></span>
						<span
							className={`block h-[2px] w-7 bg-black transition-transform duration-300 ${isActive ? "-rotate-45 -translate-y-2.5" : ""}`}
						></span>
					</div>
				</div>
			</div>
